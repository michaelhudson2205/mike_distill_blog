...
### Interactive Map
Please have a play around with the interactive map rendered above. The controls in the top-left corner are Zoom in, Zoom out, Search, and Reset View. You can zoom in and out with your mouse wheel, and left-click and hold then drag the map around to pan. The control in the top-right is the layer control and from there you can select different base maps, and select different Keyworkers to show/hide the markers. The search function lets the user search for any text contained in the labels that display when you hover over a marker. Of course, its hard to use the search function when you don't know what data is available but just try typing in different letters or numbers and you'll see how it produces selections for you. Click on any entry for the map to focus on that particular marker. Zoom in, zoom out, pan, have fun. Think of ways an interactive map could be of use to you and send me your ideas.
